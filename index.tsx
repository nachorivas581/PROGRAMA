...
// Reproductor basado en WebView con HLS.js (funciona en Android 5+)
function ReproductorWebView({ url, contentFit }: { url: string; contentFit: 'contain' | 'fill' }) {
  ...
  return (
    <WebView
                ...
                if (Hls.isSupported()) {
                  ...
                  hls.on(Hls.Events.MANIFEST_PARSED, function() {
                    video.play().catch(function(e) {
                      console.warn('Autoplay falló, esperando interacción');
                    });
                  });
                  ...
                  document.body.innerHTML = '<p style="color:white;font-size:18px;">Reproductor no soportado</p>';
                }
              </script>
            </body>
          </html>
      ...
      androidLayerType="software" // Mejor compatibilidad en dispositivos viejos
      startInLoadingState={true}
      renderLoading={() => (
        <View style={styles.loaderFondo}>
          <ActivityIndicator size="large" color="#6366f1" />
          <Text style={styles.textCarga}>Cargando reproductor...</Text>
        </View>
      )}
    />
  );
...
export default function App() {
  ...
  const alRecibirMensajeWeb = (event: WebViewMessageEvent) => {
    ...
    if (data.includes('.m3u8') || data.includes('.mpd')) {
      setLinkM3u8(data);
      setCazando(false);
    }
  };
  ...
  const contentFit = relacionAspecto === 'Ajustar' ? 'contain' : 'fill';
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Cazador de M3U8 (WebView oculto) */}
...
// Estilos (igual que antes, solo añado el de loader)
