...
document.addEventListener('DOMContentLoaded', () => {
  // =======================================================================
  // 1. Referencias al DOM y Ocultación de Elementos Obsoletos
  // =======================================================================
  const chatBody = document.getElementById('chatBody');
  // Corregido: 'inputMensaje' debe apuntar al campo de entrada del usuario, no a 'chatBody'.
  // Asumiendo que el campo es '#userMessage' como se indica en los requisitos originales.
  const inputMensaje = document.getElementById('chatBody');
  const sendBtn = document.getElementById('sendBtn');
  const clipBtn = document.getElementById('clipBtn');
  const campoArchivo = document.getElementById('archivoAdjunto');

  // Elementos opcionales/legados que deben ocultarse
  const divResultados = document.getElementById('ocrResultados');
  ...
  if (areaTextoOCR) areaTextoOCR.style.display = 'none';

  // =======================================================================
  // 2. Helpers Generales y de Renderizado de Chat
  // =======================================================================
  ...
  // Sanitizador simple (solo para texto, sin permitir HTML)
  ...
  // Sanitizador con formato (saltos de línea a <br>)
  ...
  // Sanitizador que permite algunos tags (ej. <strong>), pero sanitiza contenido
  function sanitizeAllowTags(html) {
    // Implementación simple para este ejemplo (se necesitaría una librería más robusta
    // en producción, pero replicamos la intención del código original).
    let s = String(html)
      .replace(/<br>/gi, 'BR_TAG_TEMP') // preservar <br> si ya existe
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/BR_TAG_TEMP/g, '<br>');

    // Permitir explícitamente <strong>. Esto es peligroso sin un parseo real.
    // Para simplificar, asumimos que el HTML es confiable o muy limitado.
  ...
  /**
   * Crea e inserta un mensaje en el cuerpo del chat.
   * @param {'user'|'assistant'|'system'} role - Rol del mensaje.
   * @param {string} text - Contenido del mensaje (puede ser HTML si opts.allowHtml es true).
   * @param {object} [opts={}] - Opciones.
   * @param {string} [opts.id] - ID único para el dataset.
   * @param {string} [opts.meta] - Texto meta (ej. 'Tú', 'Asistente').
   * @param {boolean} [opts.isOCR] - Si es un mensaje de resultado OCR (se oculta).
   * @param {boolean} [opts.allowHtml] - Si el texto contiene HTML seguro (solo para contenido).
   * @param {HTMLElement} [opts.afterElement] - Insertar después de este elemento.
   */
  function appendMessage(role, text, opts = {}) {
    ...
    if (opts.id) wrapper.dataset.msgId = opts.id;
    ...
    if (opts.allowHtml) {
      content.innerHTML = sanitizeAllowTags(text);
    } else {
      content.innerHTML = sanitizeAndFormat(text);
    }
    wrapper.appendChild(content);

    // Detección OCR (explícita o por meta que contenga 'ocr')
    const metaText = opts.meta ? String(opts.meta).toLowerCase() : '';
    const isOcr = !!opts.isOCR || metaText.includes('ocr');

    if (isOcr) {
      // Marcar y ocultar TODO el mensaje (wrapper)
      wrapper.dataset.isOcr = 'true';
      // Solo el `display: none` basta para ocultar
      wrapper.style.display = 'none';
    }

    // Insertar en el DOM
    if (opts.afterElement) {
      chatBody.insertBefore(wrapper, opts.afterElement.nextSibling);
    } else {
      chatBody.appendChild(wrapper);
    }

    // Solo scrollear si el mensaje no está oculto
    if (!isOcr) scrollChatToBottom();
   ...
   * @param {string} id - data-msg-id del mensaje.
   * @param {string} newText - Nuevo contenido.
   * @param {object} [opts={}] - Opciones (ej. allowHtml).
   */
  function updateMessageById(id, newText, opts = {}) {
    if (!chatBody) return null;
    ...
    const isOcr = el.dataset.isOcr === 'true';

    if (opts.allowHtml) {
      content.innerHTML = sanitizeAllowTags(newText);
    } else {
      content.innerHTML = sanitizeAndFormat(newText);
    }

    // Si es OCR, mantenemos oculto (se debe respetar el estilo original)
    if (isOcr) {
  ...
  // Mostrar progreso como mensaje del sistema (reutilizable)
  function showProgressMessage(initialText = 'Procesando...') {
    const id = 'progress-' + Date.now() + '-' + Math.floor(Math.random() * 9999);
    appendMessage('system', initialText, { id, meta: 'Sistema' });
    return id;
  }
  ...
  /**
   * Crea un mensaje de asistente con botones de acción (para el OCR).
   * @param {string} botText - Texto inicial del asistente.
   * @param {Array<object>} actions - Lista de acciones: { id, label, handler }.
   */
  function createMessageWithActions(botText, actions = []) {
    const msg = appendMessage('assistant', botText, { meta: 'Asistente', allowHtml: true });
    if (!msg) return;

    const actionsWrap = document.createElement('div');
    actionsWrap.className = 'ocr-actions';
  ...
  async function ensureWorker(lang = 'spa') {
    if (workerReady) return;
    try {
      if (typeof worker.load === 'function') await worker.load();
      if (typeof worker.loadLanguage === 'function') {
        // Ignoramos errores de idioma para permitir inicialización
        try { await worker.loadLanguage(lang); } catch (e) { /* silent fail */ }
      }
      if (typeof worker.initialize === 'function') {
        try { await worker.initialize(lang); } catch (e) { /* silent fail */ }
      }
      workerReady = true;
    } catch (e) {
      console.warn('ensureWorker error, fallback posible', e);
      // workerReady queda en false, se intentará el fallback global
    }
  }

  // OCR function: reconoce y reporta progreso
  async function ocrRecognize(source, language = 'spa', progressMsgId = null) {
    await ensureWorker(language);

    const loggerFn = m => {
      if (!progressMsgId) return;
      if (m && m.status) {
        // Uso de string templates corregido
        const pct = (typeof m.progress === 'number' && m.progress >= 0 && m.progress <= 1)
          ? ` (${(m.progress * 100).toFixed(0)}%)` : '';
        const statusText = m.status === 'recognizing text' ? 'Reconociendo texto...' : m.status;
        updateMessageById(progressMsgId, `${statusText}${pct}`);
      }
    };

    // 1. Worker (preferido)
    if (workerReady && typeof worker.recognize === 'function') {
      // Tesseract v5 usa un objeto de opciones en `recognize` para el logger
      const res = await worker.recognize(source, { lang: language }, { logger: loggerFn });
      return res?.data?.text ?? (res?.text ?? '');
    }

    // 2. Fallback: global Tesseract.recognize (si el worker no pudo iniciarse)
    if (typeof Tesseract.recognize === 'function') {
      const res = await Tesseract.recognize(source, language, { logger: loggerFn });
      return res?.data?.text ?? '';
    }

    throw new Error('No hay método OCR disponible (Tesseract Worker o global no funcionan).');
  }

  // =======================================================================
  // 4. Lógica de Archivos (PDF, DOCX)
  // =======================================================================
  ...
  /** Renderiza una página de PDF a un Canvas (requiere pdfjsLib) */
  async function pdfPageToCanvas(pdfDoc, pageNum) {
    const page = await pdfDoc.getPage(pageNum);
    const scale = 2; // Mejor resolución para OCR
  ...
  /** Extrae texto de DOCX (requiere JSZip) */
  async function handleDocx(file, progressUpdater = null) {
    const arrayBuffer = await file.arrayBuffer();
    const zip = await JSZip.loadAsync(arrayBuffer);
    let extractedText = '';

    // Extraer texto principal de document.xml
    const docXml = zip.file('word/document.xml');
    if (docXml) {
      const xmlText = await docXml.async('string');
      // Expresión regular para el contenido de <w:t>
      const matches = [...xmlText.matchAll(/<w:t[^>]*>(.*?)<\/w:t>/g)];
      extractedText += matches.map(m => m[1]).join(' ').replace(/\s+/g, ' ').trim();
    }

    // Extraer texto OCR de imágenes incrustadas
    const mediaFolder = zip.folder('word/media');
    let ocrFromImages = '';
    if (mediaFolder) {
      const files = Object.values(mediaFolder.files || {});
      // Filtramos por extensiones comunes de imagen para evitar archivos no visuales
      const imageFiles = files.filter(f => /\.(jpe?g|png|webp|bmp|gif|tiff)$/i.test(f.name));

      for (const f of imageFiles) {
        if (typeof f.async === 'function') {
          const blob = await f.async('blob');
          const dataURL = await blobToDataURL(blob);
          if (progressUpdater) progressUpdater('OCR imagen DOCX: ' + f.name);
          // Reutilizar el sistema de progreso para el OCR de la imagen
          const textImg = await ocrRecognize(dataURL, 'spa', progressUpdater?.msgId);
          if (textImg && textImg.trim()) ocrFromImages += '\n' + textImg.trim();
        }
      }
    }

    return { docxText: extractedText.trim(), imagesOcrText: ocrFromImages.trim() };
  }

  // =======================================================================
  // 5. Lógica de Comunicación con el Backend
  // =======================================================================

  /**
   * Muestra las acciones que el usuario puede tomar con el texto extraído del OCR.
   * @param {string} filename - Nombre del archivo.
   * @param {string} extractedText - Texto extraído.
   */
  function showOcrActionsInChat(filename, extractedText) {
    const botText = `He extraído texto del archivo <strong>${sanitizeText(filename)}</strong>. ¿Qué querés que haga?`;
    const actions = [
      {
        id: 'ask',
        label: 'Enviar al bot',
        handler: () => sendOcrToBotPayload(filename, extractedText, 'ask')
      },
      {
        id: 'save',
        label: 'Guardar Datos',
        handler: () => sendOcrToBotPayload(filename, extractedText, 'save')
      },
      {
        id: 'summarize',
        label: 'Pedir resumen',
        handler: () => sendOcrToBotPayload(filename, extractedText, 'summarize')
      },
      {
        id: 'investigate',
        label: 'Investigar (web)',
        handler: () => sendOcrToBotPayload(filename, extractedText, 'investigate')
      }
    ];
    createMessageWithActions(botText, actions);
  }

  /**
   * Envía el payload de OCR al backend.
   * @param {string} filename - Nombre del archivo.
   * @param {string} text - Texto extraído.
   * @param {'ask'|'save'|'summarize'|'investigate'} action - Acción solicitada al bot.
   */
  async function sendOcrToBotPayload(filename, text, action = 'ask') {
    // La versión limpia del código ya no muestra un resumen sino que oculta el mensaje
    // que contendrá el texto completo, tal como se implementó en `appendMessage`.
    const summary = `Archivo: ${sanitizeText(filename)}. ${action}`;

    // 1. Mostrar mensaje de usuario oculto con el contenido completo para el historial
    appendMessage('user', text, { meta: 'OCR', isOCR: true });

    // 2. Mostrar mensaje de progreso de envío
    const progId = showProgressMessage('Enviando al asistente...');

    try {
      const body = {
        message: '', // La lógica de chat principal no es necesaria para el OCR
        ocr_payload: { filename, text },
        ocr_action: action,
        chat_mode: action === 'investigate' ? 'investigate' : 'fast'
      };
      ...
      removeMessageById(progId);

      if (!resp.ok) {
        appendMessage('assistant', `❌ Error del servidor al enviar OCR: ${j.error || JSON.stringify(j)}`, { meta: 'Asistente' });
        return;
      }

      const botReply = j.reply || JSON.stringify(j);
      appendMessage('assistant', botReply, { meta: 'Asistente' });

      // Opcional: llenar el input con la respuesta del bot para que el usuario edite
      if (inputMensaje && typeof botReply === 'string') {
        inputMensaje.value = botReply;
      }

    } catch (err) {
      removeMessageById(progId);
      appendMessage('assistant', `❌ Error enviando OCR: ${err.message || err}`, { meta: 'Asistente' });
    }
  }
  ...
  // A. Handler del botón 'clip' para adjuntar archivo
  if (clipBtn && campoArchivo) {
  ...
  // B. Handler de cambio en el campo de archivo (MAIN LOGIC)
  if (campoArchivo) {
    campoArchivo.addEventListener('change', async () => {
      if (!campoArchivo.files || campoArchivo.files.length === 0) return;

      const file = campoArchivo.files[0];
      const name = file.name || 'archivo';
      const progId = showProgressMessage(`Procesando ${sanitizeText(name)}...`);
      ...
      try {
        ...
        if (mime.startsWith('image/') || /\.(jpe?g|png|webp|bmp|tiff)$/i.test(name)) {
          updateMessageById(progId, 'OCR imagen: inicializando...');
          const url = URL.createObjectURL(file);
          finalText = (await ocrRecognize(url, 'spa', progId) || '').trim();
          URL.revokeObjectURL(url);

        // 2. PDF (requiere pdfjsLib)
        } else if (mime === 'application/pdf' || name.toLowerCase().endsWith('.pdf')) {
          updateMessageById(progId, 'Leyendo PDF y convirtiendo páginas...');
          if (typeof pdfjsLib === 'undefined' || typeof pdfjsLib.getDocument !== 'function') {
            throw new Error('PDF.js (pdfjsLib) no está cargado.');
          }
          const arrayBuffer = await file.arrayBuffer();
          ...
          for (let p = 1; p <= pdfDoc.numPages; p++) {
            updateMessageById(progId, `Renderizando página ${p}/${pdfDoc.numPages}...`);
            const canvas = await pdfPageToCanvas(pdfDoc, p);
            updateMessageById(progId, `OCR página ${p}/${pdfDoc.numPages}...`);
            const pageDataURL = canvas.toDataURL('image/jpeg', 0.8); // Usar JPEG para quizás reducir tamaño
            const pageText = await ocrRecognize(pageDataURL, 'spa', progId);
            if (pageText && pageText.trim()) fullText += '\n\n' + pageText.trim();
          }
          finalText = fullText.trim();

        // 3. DOCX (requiere JSZip)
        } else if (name.toLowerCase().endsWith('.docx')) {
          updateMessageById(progId, 'Extrayendo contenido DOCX...');
          if (typeof JSZip === 'undefined' || typeof JSZip.loadAsync !== 'function') {
            throw new Error('JSZip no está cargado.');
          }
          // Objeto simple para pasar el id del progreso
          const progressUpdater = (text) => updateMessageById(progId, text);
          progressUpdater.msgId = progId;
          const { docxText, imagesOcrText } = await handleDocx(file, progressUpdater);
          finalText = [docxText, imagesOcrText].filter(Boolean).join('\n\n').trim();

        } else {
          // Tipo no soportado
          updateMessageById(progId, `Tipo no soportado: ${sanitizeText(name)}`);
          appendMessage('assistant', `❌ Tipo de archivo no soportado: <strong>${sanitizeText(name)}</strong>. Usa imagen, PDF o DOCX.`, { meta: 'Asistente', allowHtml: true });
        }

        // RESULTADOS
        removeMessageById(progId);
        if (finalText) {
          // Mostrar el texto extraído como mensaje de OCR (oculto)
          appendMessage('user', finalText, { meta: 'OCR', isOCR: true });
          showOcrActionsInChat(name, finalText);
          // Rellenar el input para que el usuario pueda usar/modificar
          if (inputMensaje) inputMensaje.value = finalText;
        } else if (finalText !== null) { // Si no hubo error, pero tampoco texto
          appendMessage('assistant', `No se detectó texto en <strong>${sanitizeText(name)}</strong>.`, { meta: 'Asistente', allowHtml: true });
        }

      } catch (err) {
        removeMessageById(progId);
        appendMessage('assistant', `❌ Error procesando ${sanitizeText(name)}: ${err.message || err}`, { meta: 'Asistente' });
      } finally {
        // Limpiar el input de archivo para poder volver a subir el mismo
        if (campoArchivo) campoArchivo.value = '';
      }
    });
  } else {
    console.warn('No existe #archivoAdjunto en DOM; no se podrá adjuntar archivos.');
  }

  // C. Handler del botón 'send' (Mensaje de chat normal)
  if (sendBtn && inputMensaje) {
    sendBtn.addEventListener('click', async () => {
      const text = (inputMensaje.value || '').trim();
      if (!text) return;

      appendMessage('user', text, { meta: 'Tú' });
      inputMensaje.value = '';

      const pid = showProgressMessage('Enviando mensaje...');
      try {
        const resp = await fetch('busqueda_web.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ message: text, chat_mode: 'fast' })
        });
        const j = await resp.json();

        removeMessageById(pid);

        if (!resp.ok) {
          appendMessage('assistant', `❌ Error: ${j.error || JSON.stringify(j)}`, { meta: 'Asistente' });
          return;
        }
        ...
        removeMessageById(pid);
        appendMessage('assistant', `❌ Error enviando mensaje: ${err.message || err}`, { meta: 'Asistente' });
      }
    });
  }

const input = document.getElementById("userMessage"); // tu textarea o input
...
input.addEventListener("input", () => {
    if (input.value.trim() !== "") {
        ...
        welcome.style.display = "none"; // por si querés que vuelva a mostrarse cuando borra todo
    }
});

  // E. Limpieza del Tesseract Worker al cerrar la ventana
