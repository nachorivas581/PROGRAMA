...
// ============================================================
// SISTEMA DE MONITOREO DE TOKENS Y CACHÉ (sin eventos window)
// ============================================================
...
// ============================================================
// EXTRACCIÓN CON PROXY (SIN WebView)
// ============================================================
...
// ============================================================
// COMPONENTE: SourceSelectorModal
// ============================================================
...
// ============================================================
// COMPONENTE: ProfileScreen (pantalla completa)
// ============================================================
...
// ============================================================
// COMPONENTE: LivePlayerMini (con callback de renovación)
// ============================================================
const LivePlayerMini = memo(({
  ...
  useEffect(() => {
    if (onStreamRenewed) {
      onStreamRenewed((newStream: string) => {
  ...
  const showPlayer = !loading && currentUrl && currentUrl.startsWith('http');
...
// ============================================================
// VISTAS: ContinueWatchingView, HistoryView, SettingsView
// ============================================================
...
// ============================================================
// COMPONENTES: WebViewPlayer (fallback), MoviesGrid, SeriesGrid, SeriesDetail
// ============================================================
...
// ============================================================
// COMPONENTE: ReproductorMejorado (con callback de renovación)
// ============================================================
...
export default function AppTV() {
    ...
    try {
      ...
      if (channel.embedSlug) {
        ...
        for (const opt of opts) {
          const resolved = await resolveStreamWithRenewal(channel.embedSlug, opt.url, isLegacyWebView || forceProxy, notifyStreamRenewed);
          if (resolved) {
            stream = resolved;
  ...
  const refreshCurrentStream = useCallback(async () => {
    ...
    try {
      const forceProxy = await AsyncStorage.getItem('forceProxy').then(v => v === 'true');
      const stream = await resolveStreamWithRenewal(currentChannel.embedSlug, currentChannel.url, isLegacyWebView || forceProxy, notifyStreamRenewed);
      if (stream) {
        setCurrentUrl(stream);
  ...
  const selectSource = useCallback(async (index: number) => {
    ...
    try {
      const forceProxy = await AsyncStorage.getItem('forceProxy').then(v => v === 'true');
      const stream = await resolveStreamWithRenewal(currentChannel?.embedSlug, sourceUrl, isLegacyWebView || forceProxy, notifyStreamRenewed);
      if (stream) {
        setCurrentUrl(stream);
      } else {
